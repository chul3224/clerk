const ACCENT = [
  { border: 'border-blue-500/25',   badge: 'bg-blue-500/10 text-blue-500'   },
  { border: 'border-violet-500/25', badge: 'bg-violet-500/10 text-violet-500'},
  { border: 'border-emerald-500/25',badge: 'bg-emerald-500/10 text-emerald-500'},
]

function MetricBadge({ label, value, accent }) {
  return (
    <div className={`flex flex-col items-center px-3 py-2 rounded-lg ${accent}`}>
      <span className="text-[10px] text-c-faint">{label}</span>
      <span className="text-xs font-bold text-c-soft mt-0.5">{value}</span>
    </div>
  )
}
...
function SummaryCard({ summary, index }) {
  ...
  return (
    <div className={`bg-c-card rounded-2xl border ${acc.border} p-5 shrink-0 w-80 flex flex-col gap-4`}>
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-bold text-c-dim uppercase tracking-widest">모델 {String.fromCharCode(65 + index)}</span>
        <div className="text-right">
          <p className="text-xs font-semibold text-c-soft">{summary.model}</p>
          <p className="text-[11px] text-c-dim">{summary.label}</p>
        </div>
      </div>
      ...
      {isError ? (
        <div className="bg-red-500/5 border border-red-500/25 rounded-xl p-3">
          <p className="text-xs text-red-500 leading-relaxed">{summary.summary}</p>
        </div>
      ) : (
        <>
          <div>
            <p className="text-[10px] font-semibold text-c-dim uppercase tracking-widest mb-2">핵심 요약</p>
            <p className="text-xs text-c-soft leading-relaxed">{summary.summary}</p>
          </div>

          {summary.key_decisions?.length > 0 && (
            <div>
              <p className="text-[10px] font-semibold text-c-dim uppercase tracking-widest mb-2">결정사항</p>
          ...
          {summary.action_items?.length > 0 && (
            <div>
              <p className="text-[10px] font-semibold text-c-dim uppercase tracking-widest mb-2">액션아이템</p>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <h3 className="text-sm font-semibold text-c-soft">3개 모델 비교</h3>
        {fastest && (
